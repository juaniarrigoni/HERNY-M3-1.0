...
server.post("/posts", (req, res) => {
  //esto lo recibimos por el body
  // el body sale del fetch del frontend
  //fetch.post('localpost...', {nombre: 'juani', apellido: 'arrigoni'})
...
server.delete("/posts", (req, res) => {
  ...
  const post = post.find((p) => p.id === id);
  if (!post) {
    return res.status(STATUS_USER_ERROR).json({ error: "No hay post" });
  }
...
server.delete("/author", (req, res) => {
  ...
  if (!author) {
    return res
      .status(STATUS_USER_ERROR)
      .json({ error: "Te olvidaste el autor" });
  }
